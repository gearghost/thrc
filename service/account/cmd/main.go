...
import (
	...
	"thrc/infra/jaeger"
	"thrc/infra/mysql"
	"thrc/service/account/conf"
	"thrc/service/account/service/grpc/proto/account"
	"thrc/service/account/service/grpc/server"
)
...
func main() {
	...
	tracer, reporter, err := jaeger.GetTracer("account grpc server")
	if err != nil {
	...
	svr := server.New(db)
	account.RegisterAccountServer(s, svr)
